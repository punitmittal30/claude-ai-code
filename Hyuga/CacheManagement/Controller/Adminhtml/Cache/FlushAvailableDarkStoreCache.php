...
class FlushAvailableDarkStoreCache extends Action
{
    /**
    ...
    {
        try {
            $this->cacheService->clearAvailableDarkStoresCache();
            $this->messageManager->addSuccessMessage(__('Available dark store cache has been cleaned.'));
        } catch (Exception $e) {
            $this->messageManager->addErrorMessage(__(
                'An error occurred while cleaning the available dark store cache: %1',
                $e->getMessage()
            ));
        }
