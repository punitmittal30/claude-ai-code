        ...
        if ($this->getRequest()->getParam('isAjax')) {
            ...
            if (!count($postItems)) {
                ...
                foreach (array_keys($postItems) as $modelid) {
                    /** @var \Pratech\ReviewRatings\Model\Media $model */
                    $model = $this->_objectManager->create(\Pratech\ReviewRatings\Model\Media::class)->load($modelid);
                    try {
                        $model->setData(array_merge($model->getData(), $postItems[$modelid]));
                        $model->save();
                    } catch (\Exception $e) {
                        $messages[] = "[Media ID: {$modelid}]  {$e->getMessage()}";
                        $error = true;
                    }
                }
            }
