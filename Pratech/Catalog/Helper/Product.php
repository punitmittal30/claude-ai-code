...
class Product
{
    ...
    public function getFormattedProductResponse(Collection $productsCollection, int $pincode = null): array
    {
        ...
        foreach ($productsCollection as $product) {
            $product = $this->formatProductForCarousel($product->getId(), $pincode);
            if (!empty($product)) {
                $products[] = $product;
            }
        }
    ...
    public function formatProductForCarousel(int $productId, int $pincode = null): array
    {
        ...
        if (!$productData['stock_info']['is_in_stock']) {
            return [];
        }
    ...
    private function getProductAttributes(ProductInterface $product, int $pincode = null): array
    {
            ...
            try {
                if ($isDropship) {
                    ...
                    $inventoryQty = $this->inventoryLocatorService
                        ->getInventoryQtyByPincode($product->getSku(), $pincode);
                    $isInventoryInStock = $inventoryQty > 0 && $productStock->getIsInStock()
                        && $product->getStatus() == 1;
                }
            } catch (Exception $e) {
        ...
        $pincodeLevelStockStatus = $isInventoryInStock ?? ($productStock->getIsInStock() && $product->getStatus() == 1);
        ...
        $productData = [
            ...
            'stock_info' => [
                ...
                'inventory_is_in_stock' => $pincodeLevelStockStatus,
...
//                'is_in_stock' => $productStock->getIsInStock() && $product->getStatus() == 1
                'is_in_stock' => $pincodeLevelStockStatus
            ],
     ...
     * @throws NoSuchEntityException
     */
    private function getAverageProductRating(ProductInterface $product): array
    {
        ...
        try {
            ...
            if ($product) {
                ...
                foreach ($productIds as $productId) {
                    ...
                    if (!empty($recommendedProduct) && $recommendedProduct['stock_info']['is_in_stock']) {
