...
class Order
{
    ...
    public function recurringOrder()
    {
        try {
            ...
            foreach ($subscriptionCollection as $subscription) {
                if ($subscription->getPaymentCode() !== "cashondelivery") {
                    continue;
                }
     ...
     * @return string
     */
    private function getValidTill(int $duration, string $durationType): string
    {
        switch ($durationType) {
            ...
            case RecurringDurationType::YEAR:
                $validTill = date('Y-m-d', strtotime(' + ' . $duration . $durationType));
                break;
            default:
